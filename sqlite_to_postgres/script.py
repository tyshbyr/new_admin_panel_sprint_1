...
class SQLiteExtractor:
    ...
    def extract_movies(self):
        ...
        curs.execute(f"SELECT {self.sqlite_table.columns} FROM {self.sqlite_table.table};")
        while True:
            data = curs.fetchmany()
            if not data:
                break
    ...
    def save_all_data(self, data):
        with self.pg_conn.cursor() as curs:
            ...
            for value in data:
                ...
                data_file.seek(0)
                curs.copy_from(data_file, self.pg_table.table, sep='\t', null='None', columns=columns)

...
class TestGenre:
    def test_row_count(sqlite_curs, pg_curs, table_name):
        ...
        pg_curs.execute(sql.SQL("SELECT COUNT (id) FROM {table}").format(
...
class TestPerson:
    ...
    def test_row_consistency(sqlite_curs, pg_curs, table_name):
        ...
        pg_curs = pg_conn.cursor()
        pg_curs.execute(
            sql.SQL("""SELECT id, full_name, created, modified
                    FROM {table} ORDER BY id""").format(
                table=sql.Identifier(table_name)))
        pg_rows = pg_curs.fetchall()
        pg_curs.close()
        assert sqlite_rows == pg_rows
...
class TestFilmwork:
    ...
    def test_row_consistency(sqlite_curs, pg_curs, table_name):
        ...
        sqlite_curs = sqlite_conn.cursor()
        sqlite_curs.execute(
            """SELECT id, title, description, creation_date, file_path, rating,
            type, created_at, updated_at  FROM '%s' ORDER BY id;""" %
            table_name)
        sqlite_rows = list(sqlite_curs.fetchall())
        result = []
        ...
        pg_curs.execute(
            sql.SQL(
                ...
                rating, type, created, modified FROM {table} ORDER BY id"""
            ).format(table=sql.Identifier(table_name)))
        pg_rows = pg_curs.fetchall()
        pg_curs.close()
        assert sqlite_rows == pg_rows
...
class TestGenreFilmWork:
    ...
    def test_row_consistency(sqlite_curs, pg_curs, table_name):
        ...
        pg_curs = pg_conn.cursor()
        pg_curs.execute(
            sql.SQL("""SELECT id, genre_id, film_work_id, created
                    FROM {table}""").format(
                table=sql.Identifier(table_name)))
        pg_rows = pg_curs.fetchall()
        pg_curs.close()
...
SQLITE_FILE = __file__.replace('tests/check_consistency/tests.py', 'db.sqlite')
dsl = {'dbname': 'movies_database', 'user': 'app',
       'password': '123qwe', 'host': '127.0.0.1', 'port': 5432}
with sqlite3.connect(SQLITE_FILE) as sqlite_conn, psycopg2.connect(
